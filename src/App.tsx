...
function AppNavigation() {
  ...
  const handleUpdateGoal = (updatedGoal: any) => {
    ...
    const contribution = updatedGoal.current - (oldGoal?.current || 0);

    if (contribution > 0) {
      setTotalSpendable(prev => prev - contribution);
      setMonthlyLimit(prev => prev - contribution);
    }
    setGoals(goals.map(g => g.id === updatedGoal.id ? updatedGoal : g));
  };

  const handleAddGoal = (newGoal: any) => {
    setGoals([...goals, { ...newGoal, id: Date.now().toString(), current: 0, suggestedDaily: 0 }]);
  };

  const handleDeleteGoal = (id: string) => {
    const goalToRefund = goals.find(g => g.id === id);
    if (goalToRefund && goalToRefund.current > 0) {
      setTotalSpendable(prev => prev + goalToRefund.current);
      setMonthlyLimit(prev => prev + goalToRefund.current);
    }
  ...
  const navTheme = React.useMemo(() => ({
    ...
    colors: { ...(darkMode ? DarkTheme.colors : DefaultTheme.colors), background: colors.background, card: colors.card, text: colors.foreground, border: colors.border, primary: colors.primary },
  }), [darkMode, colors]);
            ...
            {!isProfileComplete ? (
              <>
                ...
                <Stack.Screen name="BudgetSetup">
                  {(props) => <BudgetSetup {...props} onComplete={checkProfileStatus} setGlobalBalances={(s:number, l:number) => {setTotalSpendable(s); setMonthlyLimit(l);}} />}
                </Stack.Screen>
              </>
            ) : (
              <>
                <Stack.Screen name="Home">
                  {(props) => <HomeDashboard {...props} userName={getCleanName()} goals={goals} totalSpendable={totalSpendable} monthlyLimit={monthlyLimit} />}
                </Stack.Screen>
                ...
                <Stack.Screen name="Settings">
                  {(props) => <Settings {...props} userName={getCleanName()} totalSpendable={totalSpendable} monthlyLimit={monthlyLimit} setTotalSpendable={setTotalSpendable} setMonthlyLimit={setMonthlyLimit} onLogout={() => auth().signOut()} />}
                </Stack.Screen>
                <Stack.Screen name="Calendar" component={SpendingCalendar} />
                <Stack.Screen name="Analytics">{(props) => <Analytics {...props} goals={goals} />}</Stack.Screen>
