...
import { useDarkMode } from '../DarkModeProvider'; // Import your custom hook
...
export default function SpendingCalendar() {
  const { darkMode: isDark } = useDarkMode(); // Use your custom hook logic
    ...
    transactions.forEach(tx => {
      ...
      if (d.getMonth() === currentMonth && d.getFullYear() === currentYear) {
        ...
        if (tx.direction === 'debit') {
          spending[day].amount += Math.abs(tx.amount);
  ...
  }, [transactions, currentMonth, currentYear, highLimit, overLimit]);
              ...
              {selectedDayExpenses.length > 0 ? selectedDayExpenses.map((tx, i) => (
                <View key={i} style={[styles.expenseItem, { borderBottomColor: theme.border }, i === selectedDayExpenses.length - 1 && { borderBottomWidth: 0 }]}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={[styles.expTime, { color: theme.subText }]}>{new Date(tx.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>
                  </View>
                  <Text style={styles.expAmt}>-₹{tx.amount}</Text>
                </View>
              )) : (
